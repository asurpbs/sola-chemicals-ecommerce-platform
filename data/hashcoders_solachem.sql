...
CREATE TABLE IF NOT EXISTS `cart_item` (
  ...
  KEY `FK_cart_items_item` (`item_id`) USING BTREE,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='to store cart items';
...
INSERT INTO `feedback` (`id`, `user_id`, `comment`, `status`, `date_created`) VALUES
	...
	(4, 2, 'Happy with my purchase! ItÃ¢â‚¬â„¢s exactly what I needed.', _binary 0x30, '2025-01-21 14:30:36'),
...
INSERT INTO `public_contact` (`id`, `name`, `email`, `message`, `date_created`, `status`) VALUES
	...
	(5, 'Pwan Madusanka', 'pawan@gmail.com', 'HeyÃ°Å¸ËœÂ!', '2025-01-21 13:54:44', _binary 0x30);
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` TRIGGER `cart_items_before_insert validity check` BEFORE INSERT ON `cart_item` FOR EACH ROW BEGIN
...
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE DEFINER=`root`@`localhost` TRIGGER `item_before_update update availability of items` BEFORE UPDATE ON `item` FOR EACH ROW BEGIN
    IF NEW.QoH < 0 THEN
        SIGNAL SQLSTATE '45000'
...
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE DEFINER=`root`@`localhost` TRIGGER `news_and_events_before_insert` BEFORE INSERT ON `news_and_events` FOR EACH ROW BEGIN
...
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE DEFINER=`root`@`localhost` TRIGGER `notification_before_insert updated update last accessed` BEFORE INSERT ON `notification` FOR EACH ROW BEGIN
IF NEW.status = 1 THEN
...
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE DEFINER=`root`@`localhost` TRIGGER `order_after_insert update item stock` AFTER INSERT ON `order` FOR EACH ROW BEGIN
...
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE DEFINER=`root`@`localhost` TRIGGER `order_before_insert avalidity check` BEFORE INSERT ON `order` FOR EACH ROW BEGIN
